...
@section('title', 'تعديل الأداة')
@section('page-title', 'تعديل الأداة')
...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 mb-2">✏️ تعديل الأداة: {{ $tool->name }}</h1>
            <p class="text-gray-600">تحديث بيانات الأداة</p>
        </div>
...
<div class="bg-white rounded-lg shadow-lg">
    <form action="{{ route('admin.tools.update', $tool->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="url" 
                               name="officialLink" 
                               id="officialLink"
                               value="{{ old('officialLink', $tool->official_link) }}" 
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
        ...
        <div class="border-b border-gray-200 pb-6">
            ...
            <div class="mb-4">
                ...
                <textarea name="desc" 
                          ...
                          placeholder="وصف موجز للأداة...">{{ old('desc', $tool->desc) }}</textarea>
            ...
            <div>
                ...
                <textarea name="long_desc" 
                          ...
                          rows="6" 
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none"
                          placeholder="وصف تفصيلي للأداة، المميزات، الاستخدامات...">{{ old('long_desc', $tool->long_desc) }}</textarea>
                @error('long_desc')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="border-b border-gray-200 pb-6">
            ...
            <div class="flex items-center space-x-6 space-x-reverse">
                <div class="flex-shrink-0">
                    <div id="image-preview" class="w-32 h-32 bg-gray-100 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-300 overflow-hidden">
                        @if($tool->image)
                            <img src="{{ asset($tool->image) }}" class="w-full h-full object-cover">
                        @else
                ...
                <div class="flex-1">
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        تحديث صورة الأداة
                        <span class="text-gray-400 text-xs">(اتركه فارغاً للإبقاء على الصورة الحالية)</span>
                    </label>
                    <input type="file" 
                           name="image" 
                           id="image"
            ...
            <div>
                ...
                <div id="alternatives-container" class="space-y-2">
                     @if($tool->alternatives)
                        @foreach($tool->alternatives as $alt)
